...
namespace CTDT.Areas.CTDT.Controllers
{
    public class GiamSatKetQuaKhaoSatCTDTAPIController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/ctdt/giam_sat_ty_le_khao_sat")]
        public async Task<IHttpActionResult> giam_sat_ty_le_khao_sat(GiamSatThongKeKetQua giamsat)
        {
        ...
        private async Task muc_do_hai_long(dynamic MucDoHaiLong, int? idctdt, int? idsurvey)
        {
            var Mucdohailong = db.answer_response
                        .Where(d => d.surveyID == idsurvey && d.id_ctdt == idctdt)
                        .AsEnumerable()
                        .AsQueryable();

...
{
    [UserAuthorize(3)]
    public class BaoCaoTongHopKetQuaKhaoSatController : Controller
    {
        dbSurveyEntities db = new dbSurveyEntities();
        // GET: CTDT/BaoCaoTongHopKetQuaKhaoSat
        ...
        public JsonResult x_loadbaocaotonghop(int id)
        {
            ...
            foreach (var item in sortedSurveyList)
            {
                ...
                if (!string.IsNullOrEmpty(CheckKey.key_class))
                {
                    ...
                    if (isStudent)
                    {
                        ...
                        var idphieu = db.survey.Where(x => x.surveyID == item.ma_phieu).FirstOrDefault();
                    ...
                    var Mucdohailong = db.answer_response.Where(d => keyClassList.Any(k => d.sinhvien.lop.ma_lop.Contains(k)) && d.id_ctdt == user.id_ctdt && d.surveyID == item.ma_phieu).AsQueryable();
                    ...
                    var maphieu = db.survey.Where(x => x.surveyID == item.ma_phieu).FirstOrDefault();
                ...
                else
                {
                    ...
                    var Mucdohailong = db.answer_response.Where(d => d.id_ctdt == user.id_ctdt && d.surveyID == item.ma_phieu).AsQueryable();
                    var responses = Mucdohailong
                        .Select(x => new { IDPhieu = x.surveyID, JsonAnswer = x.json_answer, SurveyJson = x.survey.surveyData })
                        .ToList();

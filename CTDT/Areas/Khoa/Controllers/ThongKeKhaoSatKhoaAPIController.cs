...
namespace CTDT.Areas.Khoa.Controllers
{
    public class ThongKeKhaoSatKhoaAPIController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/khoa/load_thong_ke_nguoi_hoc")]
        public IHttpActionResult load_charts_nguoi_hoc(FindChartsTyLeKhaoSat find)
        {
            ...
            foreach (var survey in query)
            {
                ...
                if (!string.IsNullOrEmpty(idphieu.key_class))
                {
                    ...
                    if (isStudent)
                    {
                        sinh_vien_thuong(ChartSurvey, find.id_ctdt, user.id_khoa, idphieu.surveyID, survey.ctdt.ten_ctdt, keyClassList);
                    }
                    else if (isStudentBySubject)
                    {
                        sinh_vien_subject(ChartSurvey, find.id_ctdt, user.id_khoa, idphieu.surveyID, survey.ctdt.ten_ctdt, survey.id_hk, keyClassList);
                    }
                }
                else
                {
                    ...
                    if (isCTDT)
                    {
                        chuong_trinh_dao_tao(ChartSurvey, find.id_ctdt, user.id_khoa, idphieu.surveyID, survey.ctdt.ten_ctdt);
                    }
                    else if (isCBVC)
                    {
                        can_bo_vien_chuc(ChartSurvey, find.id_ctdt, user.id_khoa, idphieu.surveyID, survey.ctdt.ten_ctdt);
                    }
                }
                Survey.Add(new
                {
        ...
        [HttpPost]
        [Route("api/khoa/load_pks_by_year")]
        public IHttpActionResult LoadPKSByYear(NamHoc namhoc)
        {
            var surveys = db.survey
                            .Where(x => x.NamHoc.ten_namhoc == namhoc.ten_namhoc)
                            .ToList();
        ...
        public async Task<IHttpActionResult> load_tan_xuat(FindAnswerSurvey f)
        {
            ...
            if (!string.IsNullOrEmpty(survey?.key_class))
            {
                ...
                if (isStudent)
                {
                    sinh_vien_thuong(ty_le_khao_sat, f.id_ctdt, user.id_khoa, survey.surveyID, query.FirstOrDefault().ctdt.ten_ctdt, keyClassList);
                }
                else if (isStudentBySubject)
                {
                    ...
                    {
                        sinh_vien_subject(ty_le_khao_sat, f.id_ctdt, user.id_khoa, survey.surveyID, query.FirstOrDefault().ctdt.ten_ctdt, firstItem.id_hk, keyClassList);
                    }
                    else
                    {
                        return Ok(new { message = "Không có dữ liệu cho học kỳ này" });
                    }

                }
                if (isStudent || isStudentBySubject)
                {
                    tan_xuat_5_muc = cau_hoi_5_muc(query, f.id_ctdt,user.id_khoa, survey.surveyID, keyClassList);
                    tan_xuat_1_lua_chon = cau_hoi_mot_lua_chon(query, f.id_ctdt, user.id_khoa, survey.surveyID, keyClassList);
                    tan_xuat_nhieu_lua_chon = cau_hoi_nhieu_lua_chon(query, f.id_ctdt, user.id_khoa, survey.surveyID, keyClassList);
                    tan_xuat_y_kien_khac = y_kien_khac(query, f.id_ctdt, user.id_khoa, survey.surveyID, keyClassList);
                }
            }
            else
            {
                ...
                if (isCTDT)
                {
                    chuong_trinh_dao_tao(ty_le_khao_sat, f.id_ctdt, user.id_khoa, survey.surveyID, query.FirstOrDefault().ctdt.ten_ctdt);
                }
                else if (isCBVC)
                {
                    can_bo_vien_chuc(ty_le_khao_sat, f.id_ctdt, user.id_khoa, survey.surveyID, query.FirstOrDefault().ctdt.ten_ctdt);
                }
                if (isCTDT || isCBVC)
                {
                    tan_xuat_5_muc = cau_hoi_5_muc(query, f.id_ctdt, user.id_khoa, survey.surveyID);
                    tan_xuat_1_lua_chon = cau_hoi_mot_lua_chon(query, f.id_ctdt, user.id_khoa, survey.surveyID);
                    tan_xuat_nhieu_lua_chon = cau_hoi_nhieu_lua_chon(query, f.id_ctdt, user.id_khoa, survey.surveyID);
                    tan_xuat_y_kien_khac = y_kien_khac(query, f.id_ctdt, user.id_khoa, survey.surveyID);
                }
            }
            var data = new
            {
        ...
        private List<object> cau_hoi_5_muc(IEnumerable<answer_response> query, int? idctdt, int? idkhoa, int? idsurvey, List<string> keyClassList = null)
        {
            var responses = query
                .Where(d => (keyClassList == null || keyClassList.Any(k => d.sinhvien.lop.ma_lop.Contains(k))) && d.ctdt.khoa.id_khoa == idkhoa && d.surveyID == idsurvey);

            if (idctdt.HasValue && idctdt != 0)
            {
                responses = responses.Where(x => x.id_ctdt == idctdt);
            }
            var AnswerSurvey = responses
        ...
        "Hoàn toàn đồng ý"
            ...
            foreach (var response in AnswerSurvey)
            {
        ...
        private List<object> cau_hoi_mot_lua_chon(IEnumerable<answer_response> query, int? idctdt, int? idkhoa, int? idsurvey, List<string> keyClassList = null)
        {
            var responses = query
                 .Where(d => (keyClassList == null || keyClassList.Any(k => d.sinhvien.lop.ma_lop.Contains(k))) && d.ctdt.khoa.id_khoa == idkhoa && d.surveyID == idsurvey);
        ...
        private List<object> cau_hoi_nhieu_lua_chon(IEnumerable<answer_response> query, int? idctdt,int? idkhoa, int? idsurvey, List<string> keyClassList = null)
        {
            var responses = query
        ...
        private List<object> y_kien_khac(IEnumerable<answer_response> query, int? idctdt, int? idkhoa,int? idsurvey, List<string> keyClassList = null)
        {
            var responses = query
                  .Where(d => (keyClassList == null || keyClassList.Any(k => d.sinhvien.lop.ma_lop.Contains(k))) && d.ctdt.khoa.id_khoa == idkhoa && d.surveyID == idsurvey);
        ...
        private void can_bo_vien_chuc(dynamic ChartSurvey, int? idctdt, int? idkhoa, int? surveyid, string tenctdt)
        {
            var cbvc = db.CanBoVienChuc
            ...
            if (idctdt.HasValue && idctdt != 0)
            {
                cbvc = cbvc.Where(x => x.id_chuongtrinhdaotao == idctdt).ToList();
            }
        ...
        private void chuong_trinh_dao_tao(dynamic ChartSurvey, int? idctdt, int? idkhoa, int? surveyid, string tenctdt)
        {
            var ctdt = db.answer_response
                                .Where(x => x.ctdt.khoa.id_khoa == idkhoa &&
                                            x.id_sv == null &&
            ...
            if (idctdt.HasValue && idctdt != 0)
            {
                ctdt = ctdt.Where(x => x.id_ctdt == idctdt);
            }
            var ctdtque = ctdt.Count();
            var DataCTDT = new
            {
                IDPhieu = surveyid,
                CTDT = tenctdt,
                TongKhaoSat = ctdtque,
                TongPhieuDaTraLoi = ctdtque,
                TongPhieuChuaTraLoi = 0,
                TyLeDaTraLoi = 100,
                TyLeChuaTraLoi = 0
            };
        ...
        private void sinh_vien_subject(dynamic ChartSurvey, int? idctdt, int? idkhoa, int? surveyid, string tenctdt, int? idhocky, List<string> keyClassList)
        {
            var sinhvienQuery = db.sinhvien
                    .Where(x => keyClassList.Any(k => x.lop.ma_lop.Contains(k)) && x.lop.ctdt.khoa.id_khoa == idkhoa);
            if (idctdt.HasValue && idctdt != 0)
            {
                sinhvienQuery = sinhvienQuery.Where(x => x.lop.ctdt.id_ctdt == idctdt);
            }
        ...
        private void sinh_vien_thuong(dynamic ChartSurvey, int? idctdt, int? idkhoa, int? surveyid, string tenctdt, List<string> keyClassList)
        {
            var sinhvienQuery = db.sinhvien
                            .Where(x => keyClassList.Any(k => x.lop.ma_lop.Contains(k)) && x.lop.ctdt.khoa.id_khoa == idkhoa);
            if (idctdt.HasValue && idctdt != 0)

...
namespace CTDT.Areas.Khoa.Controllers
{
    public class BaoCaoTongHopKetQuaKhaoSatKhoaAPIController : ApiController
    {
            ...
            foreach (var item in survey)
            {
                ...
                if (!string.IsNullOrEmpty(CheckKey.key_class))
                {
                    ...
                    if (isStudent)
                    {
                        ...
                        sinh_vien(DataList, find.id_ctdt, user.id_khoa,item.ma_phieu, keyClassList);
                    }
                    else if (isStudentBySubject)
                    {
                        sinh_vien_subject(DataList, find.id_ctdt, user.id_khoa, item.ma_phieu, keyClassList);

                    }
                    if (isStudent || isStudentBySubject)
                    {
                        muc_do_hai_long(MucDoHaiLong, find.id_ctdt,user.id_khoa, item.ma_phieu);
                    }
                }
                else
                {
                    ...
                    if (isCTDT)
                    {
                        chuong_trinh_dao_tao(DataList, find.id_ctdt, user.id_khoa, item.ma_phieu);
                    }
                    else if (isCBVC)
                    {
                        can_bo_vien_chuc(DataList, find.id_ctdt, user.id_khoa, item.ma_phieu);
                    }
                    if (isCTDT || isCBVC)
                    {
                        muc_do_hai_long(MucDoHaiLong, find.id_ctdt, user.id_khoa, item.ma_phieu);
        ...
        private void can_bo_vien_chuc(dynamic DataList, int? idctdt, int? idkhoa ,int? idsurvey)
        {
            var cbvc = db.CanBoVienChuc
                            .Where(x => x.ctdt.id_khoa == idkhoa);
            if(idctdt.HasValue && idctdt != 0)
            {
                cbvc = cbvc.Where(x => x.id_chuongtrinhdaotao == idctdt);
            }
        ...
        private void chuong_trinh_dao_tao(dynamic DataList, int? idctdt,int? idkhoa, int? idsurvey)
        {
            var ctdt = db.answer_response
                            .Where(x => x.ctdt.id_khoa == idkhoa && x.id_sv == null);
            if (idctdt.HasValue && idctdt != 0)
            {
        ...
        private void sinh_vien_subject(dynamic DataList, int? idctdt, int? idkhoa,int? idsurvey, List<string> keyClassList)
        {
            ...
            var TotalIsKhaoSat = sinhvienQuery.Count(sv => db.answer_response
        ...
        private void muc_do_hai_long(dynamic MucDoHaiLong, int? idctdt, int? idkhoa ,int? idsurvey, List<string> keyClassList = null)
        {
            var Mucdohailong = db.answer_response
                     .Where(d => d.ctdt.id_khoa == idkhoa && d.surveyID == idsurvey)
                     .AsEnumerable()

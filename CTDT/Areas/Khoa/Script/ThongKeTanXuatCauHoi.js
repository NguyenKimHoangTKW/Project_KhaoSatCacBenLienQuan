...
$(document).on('click', '#fildata', async function () {
    ...
    if (id) {
        ...
        try {
            body.show();
            await loc_tan_xuat(id, hocky, ctdt);
            hideLoading();
            var type = "success"
...
// Load Câu hỏi
async function loc_tan_xuat(tenphieu, hocky,ctdt) {
    const res = await $.ajax({
        url: '/api/khoa/load_tan_xuat_dap_an',
        type: "POST",
        data: {
            id_survey: tenphieu,
            name_hoc_ky: hocky,
